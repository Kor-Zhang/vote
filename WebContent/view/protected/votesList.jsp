			...
			<c:forEach items="${votes.pageVotes}" var="v">
				<tr>
					<td class="firstTd">
						...
						<a class="vote_icon" href="<c:url value='/votes/votesAction!selectVoteWithCustomField.action?forward=voteInfo&id=${v.id}'></c:url>">
							${v.theme}
							<!-- 描述 -->
							<div class="drecDiv">
...
<!-- 									private Integer itemNum;//vote選項數目
					...
					<td class="secondTd">
						<a class="join_icon" href="<c:url value='/votes/votesAction!selectVoteWithCustomField.action?forward=voteInfo&id=${v.id}'></c:url>">我要参与</a>
					</td>
				</tr>
			</c:forEach>
	...
	<div class="pager">
		...
		<c:if test="${votes.page gt 1}"><!-- 下载已经是最后一页 -->
			<a href="<c:url value='/votes/votesAction!selectVoteWithCustomFieldByPageAndKW.action?page=${votes.page-1}'></c:url>">上一页</a>
		</c:if>
		<c:if test="${votes.page lt votes.maxPage}"><!-- 下载已经是最后一页 -->
			<a href="<c:url value='/votes/votesAction!selectVoteWithCustomFieldByPageAndKW.action?page=${votes.page+1}'></c:url>">下一页</a>
		</c:if>
